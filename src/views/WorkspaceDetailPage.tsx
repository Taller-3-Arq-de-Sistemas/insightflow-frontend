...
import { useWorkspaceDetails } from "@/hooks/useWorkspacesDetails";
...
export const WorkspaceDetailPage: React.FC<WorkspaceDetailPageProps> = ({
  ...
  const LOGGED_IN_USER_ID =
    ...
    "d084f70c-238d-44a3-a7d0-1a7795325c34";

  // Manejo de estado de carga
  if (isLoading) {
    return (
      <div className="p-12 text-center">Cargando detalles del espacio...</div>
    );
  }
  ...
  if (!workspace) {
    return (
      <div className="p-12 text-center text-gray-500">
        Espacio de trabajo no encontrado o datos inválidos.
      </div>
    );
  }
    ...
    <div className="container mx-auto py-12">
      ...
      <div className="mt-12">
        <h2 className="text-2xl font-semibold mb-4 border-b pb-2">
          Contenido (Documentos y Tareas)
        </h2>
        <div className="bg-gray-50 p-6 rounded-lg border">
          <p className="text-gray-500">{workspace.description}</p>
          <p className="mt-4 text-sm">
            <b>Integración Pendiente:</b> Listado de documentos y tareas del
            `DOCUMENTS SERVICE` y `TASKS SERVICE`.
          </p>
        </div>
      </div>
    </div>

...
export const WorkspacesServices = {
  ...
  async fetchWorkspaces(userId: string): Promise<WorkspaceListItemResponse[]> {
    try {
      const response = await workspacesClient.get<WorkspaceListItemResponse[]>(
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(
          error.response?.data?.Message ||
            "Error al obtener los espacios de trabajo."
        );
      }
      throw error;
    }
  ...
  async getWorkspaceById(Id: string): Promise<WorkspaceResponse> {
    try {
      const response = await workspacesClient.get<WorkspaceResponse>(
        `workspaces/${Id}`
      );
      return response.data;
    } catch (error) {
      if (axios.isAxiosError(error)) {
        throw new Error(
          error.response?.data?.Message || "Espacio de trabajo no encontrado."
        );
      }
      throw error;
    }
  ...
  async deleteWorkspace(Id: string): Promise<void> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(
          error.response?.data?.Message ||
            "Fallo la eliminación del espacio de trabajo."
        );
      }
      throw error;
    }
